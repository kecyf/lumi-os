...
async function seedDemoWorkspace(
  ...
  for (const object of DEMO_SCHEMA.objects) {
    ...
    const { error } = await client.from('properties').insert(rows);
  ...
  for (const [index, view] of DEMO_VIEWS.entries()) {
    ...
    if (!objectId) continue;
    const { error } = await client.from('views').insert({
      object_id: objectId,
      name: view.name,
      kind: 'grid',
      filters: view.filters,
      visibility: view.visibility ?? 'collaborative',
      owner_email: view.ownerEmail ?? null,
      icon: view.icon ?? null,
...
export async function loadWorkspace(objectParam?: string): Promise<WorkspaceSnapshot> {
  ...
  try {
    ...
    const { data: views, error: viewsError } = await client
      .from('views')
      .select('id, object_id, name, filters, visibility, owner_email, icon, icon_color, position')
      .eq('object_id', objectRow.id)
...
export async function replaceViews(
  ...
  let deleteQuery = client.from('views').delete().eq('object_id', object.id);
